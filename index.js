...
;(async () => {
  ...
  try {
    ...
    const currArticles = await fetch(`https://api.github.com/repos/xiao555/blog-articles/contents/articles.json`).then(r => r.json())
    const currArticlesMap = JSON.parse(new Buffer(currArticles.content, 'base64').toString()).reduce((pre, cur) => {
      pre[cur.id] = cur
      return pre
    }, {})
    ...
    let res = await fetch(`${articleRepoLink}/contents/articles.json`, {
      ...
      body: JSON.stringify({
        ...
        sha: currArticles.sha
      })
    }).catch(e => console.error(e))
    if (res.ok) {
      console.log('发布成功！')
    } else {
      console.error('发布失败：', res.statusText)
    }
  } catch (e) {
    console.log(e);
  }
